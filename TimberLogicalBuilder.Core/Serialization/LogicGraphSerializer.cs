...
public static class LogicGraphSerializer
{
  ...
  {
    if (node.NodeType == NodeType.Relay) return SerializeRelay(node);
    if (node.NodeType == NodeType.Memory) return SerializeMemory(node);
    if (node.NodeType == NodeType.Timer) return SerializeTimer(node);
    if (node.NodeType == NodeType.Indicator) return SerializeIndicator(node);
    if (node.NodeType == NodeType.Lever) return SerializeLever(node);
    if (node.NodeType == NodeType.HttpLever) return SerializeHttpLever(node);
    if (node.NodeType == NodeType.HttpAdapter) return SerializeHttpAdapter(node);
    
    // This really shouldn't happen, we're only called by Serialize(), which checks for empty nodes.
    if (node.NodeType == NodeType.Empty) throw new ArgumentOutOfRangeException("Tried to serialize an empty node");
